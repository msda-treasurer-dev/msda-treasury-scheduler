...
const AVATAR_COLORS = ["#4F46E5","#7C3AED","#0891B2","#059669","#D97706","#DC2626","#2563EB","#DB2777","#BE185D","#65A30D","#9333EA"];
  ...
  const exportToPDF = () => {
    ...
    const rows = SATURDAYS.map((sat, i) => {
      ...
      const names = assigned.length === 2
        ? assigned.join(" & ")
        : assigned.length === 1
        ? `${assigned[0]} (1 person only)`
        : "Not yet assigned";
      const covered = assigned.length === 2;
      return `
        <tr>
          <td style="padding:12px 16px; border-bottom:1px solid #E2E8F0; font-weight:600; color:#0F172A;">
            ${sat.toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
          </td>
          ...
          <td style="padding:12px 16px; border-bottom:1px solid #E2E8F0; text-align:center;">
            <span style="display:inline-block; padding:3px 12px; border-radius:20px; font-size:12px; font-weight:600; background:${covered ? "#F0FDF4" : "#FEF2F2"}; color:${covered ? "#16A34A" : "#EF4444"}; border:1px solid ${covered ? "#BBF7D0" : "#FECACA"};">${covered ? "✓ Covered" : "✗ Not fully covered"}</span>
          </td>
        </tr>`;
    }).join("");
    printWindow.document.write(`<!DOCTYPE html><html><head><title>MSDA Treasury — Sabbath Schedule</title><style>* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', sans-serif; background: #fff; color: #1E293B; padding: 40px; } .header { display: flex; align-items: center; gap: 20px; margin-bottom: 32px; border-bottom: 2px solid #E2E8F0; padding-bottom: 20px; } .header img { width: 64px; height: 64px; object-fit: contain; border-radius: 10px; } .header-text h1 { font-size: 22px; font-weight: 700; color: #0F172A; } .header-text p { font-size: 13px; color: #94A3B8; margin-top: 4px; } table { width: 100%; border-collapse: collapse; } thead th { background: #F8FAFC; padding: 12px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #64748B; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 2px solid #E2E8F0; } .footer { margin-top: 32px; font-size: 11px; color: #94A3B8; text-align: center; }</style></head><body><div class="header"><img src="https://i.ibb.co/DXxq68q/MSDA-Logo.jpg" alt="MSDA Logo" /><div class="header-text"><h1>Mississauga SDA Treasury Team</h1><p>Sabbath Coverage Schedule — Generated ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p></div></div><table><thead><tr><th>Date</th><th>On Duty</th><th style="text-align:center;">Status</th></tr></thead><tbody>${rows}</tbody></table><div class="footer">Generated by MSDA Treasury Scheduler · msdatreasuryscheduler.netlify.app</div><script>window.onload = () => { window.print(); }<\/script></body></html>`);
...
function wb(type) {
  ...
  if (type === "pdf") return { ...base, background: "#EFF6FF", border: "1.5px solid #BFDBFE", color: "#1D4ED8" };
  return base;
}
