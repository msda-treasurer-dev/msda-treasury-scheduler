...
function getSaturdays(count = 9) {
  const saturdays = [];
  const start = new Date(2026, 7, 1); // ✏️  Change start date here if needed
...
export default function TreasuryScheduler() {
  ...
  const exportToPDF = () => {
    ...
    const rows = SATURDAYS.map((sat, i) => {
      ...
      return `
        <tr>
          <td style="padding:10px 16px; border-bottom:1px solid #E2E8F0; font-weight:600; color:#0F172A; white-space:nowrap;">
            ${sat.toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
          </td>
          <td style="padding:10px 16px; border-bottom:1px solid #E2E8F0; color:#1E293B;">
            ${names}
          </td>
        </tr>`;
    }).join("");

    printWindow.document.write(`<!DOCTYPE html><html><head><title>Sabbath Schedule</title><style>* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: 'Segoe UI', sans-serif; background: #fff; color: #1E293B; padding: 40px; } .header { display: flex; align-items: center; gap: 20px; margin-bottom: 28px; border-bottom: 2px solid #E2E8F0; padding-bottom: 20px; } .header img { width: 56px; height: 56px; object-fit: contain; border-radius: 10px; } .header-text h1 { font-size: 20px; font-weight: 700; color: #0F172A; } .header-text p { font-size: 13px; color: #64748B; margin-top: 4px; } table { width: 100%; border-collapse: collapse; } thead th { background: #F8FAFC; padding: 10px 16px; text-align: left; font-size: 11px; font-weight: 700; color: #64748B; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 2px solid #E2E8F0; } @media print { body { padding: 20px; } @page { margin: 1cm; } }</style></head><body><div class="header"><img src="https://i.ibb.co/DXxq68q/MSDA-Logo.jpg" alt="MSDA Logo" /><div class="header-text"><h1>Mississauga SDA Treasury Team</h1><p>Sabbath Coverage Schedule — ${monthRange}</p></div></div><table><thead><tr><th>Date</th><th>On Duty</th></tr></thead><tbody>${rows}</tbody></table><script>window.onload = () => { window.print(); }<\/script></body></html>`);
